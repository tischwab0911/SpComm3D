...
void read_bin_parallel_distribute(
    ...
    idx_t &grows, idx_t &gcols,
    idx_large_t &gnnz, // global size information (#rows, #cols, nnz)
    idx_large_t &lnnz, // local nnz
    std::vector<int> &rpvec2D,
    std::vector<int> &cpvec2D, // row/col partition ? (2D grid)
    MPI_Comm worldcomm, MPI_Comm xycomm,
    MPI_Comm zcomm // communicators (3D Cartesian, 2D (XY) , 1D (Z))
) {
  ...
  if (myxyrank == 0) {
    rpvec2D.resize(grows);
    cpvec2D.resize(gcols);
  ...
  idx_large_t my_nnz_start =
      myxyrank > nnz_ppR
          ...
          : (nnz_pp + 1) * myxyrank;
  idx_large_t my_nnz_end =
      my_nnz_start + (myxyrank >= nnz_ppR ? nnz_pp : nnz_pp + 1);
  ...
  MPI_Alltoall(sendCnts.data(), 1, MPI_INT, recvCnts.data(), 1, MPI_INT,
               xycomm);
  for (int i = 1; i <= xysize; ++i)
...
void read_bin_parallel_distribute_csr(std::string filename, csrMat &Sloc,
                                      std::vector<int> &rpvec2D,
                                      std::vector<int> &cpvec2D,
                                      MPI_Comm worldcomm, MPI_Comm xycomm,
                                      MPI_Comm zcomm) {}
void read_bin_parallel_distribute_coo(std::string filename, cooMat &Sloc,
                                      std::vector<int> &rpvec2D,
                                      std::vector<int> &cpvec2D,
                                      MPI_Comm worldcomm, MPI_Comm xycomm,
                                      MPI_Comm zcomm) {
  ...
  if (myzrank == 0) {
    // Note that there can be several nodes with myzrank=0!!
    ...
    for (size_t i = 0; i < lnnz; ++i) {
      Sloc.ii[i] = myelms[i].row;
      Sloc.jj[i] = myelms[i].col;
